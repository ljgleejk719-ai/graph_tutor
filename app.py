...
def format_sympy_expr(f_expr):
    ...
    # SymPy의 latex 함수를 사용하여 안전하게 LaTeX 문자열을 생성합니다.
    from sympy.printing.latex import print_latex
    from io import StringIO
    
    output = StringIO()
    print_latex(f_expr, output)
    latex_str = output.getvalue().replace('\n', '').strip()
...
# -----------------------------------------------------
# 배경 이미지 파일 경로 (GitHub에 업로드한 파일 이름을 사용)
...
if submit_button:
    ...
    contents_to_send = [
        ...
        {"mime_type": "text/plain", "text": "위는 학생이 작성한 증감표입니다. 아래는 학생이 그린 그래프입니다. AI는 좌표축 배경 이미지를 기준으로 학생의 드로잉을 분석해야 합니다."},
